...
import { testReduxCode } from '../reactTestCode';

# redux

## A.redux实现原理:context全局分发，订阅模式以通知React组件重新渲染
...
<ImageRender alt='redux-总览' path='/react/redux-总览.png' />
...
总览：
```js
export { Provider, ReactReduxContext, connect, useDispatch, createDispatchHook, 
useSelector, createSelectorHook, useStore, createStoreHook, shallowEqual };
```
...
### A-4.react-redux 是如何关联起 redux 和 react 的？
...
<CodePannel code={testReduxCode} language='html' name='testReduxCode' height='800px' />
...
<ImageRender alt='redux异步' path='/react/redux异步.png' />
