...
### A-1.流程图-commit阶段-beforeMutation和mutation阶段
<Mermaid>
```mermaid
flowchart TD
...
## B.commitRoot()是commit阶段工作的起点
### B-1.重点-performConcurrentWorkOnRoot在render结束之后调用finishConcurrentRender
<HighlightText>
...
### B-2.1.初始化时候:finishConcurrentRender-->调用commitRoot 3
<CodeBlock>
```js
...
### B-3.更新时候:performSyncWorkOnRoot-->commitRoot
<CodeBlock>
```js
...
### B-5.commitRoot 方法是commit阶段工作的起点-->commitRootImpl
<CodeBlock>
```js
...
### C-1.appendAllChildren发生在render阶段
...
### D-1.commitBeforeMutationEffects
大体代码逻辑：
```js
...
### E-4.FunctionComponent mutation
...
### E-6.HostComponent mutation
当fiber.tag为HostComponent，会调用commitUpdate，你可以在这里看到commitUpdate源码。
...
## F.commit 阶段之-layout阶段-执行DOM操作后
...
### F-1.与前两个阶段类似，layout阶段也是遍历effectList，执行函数
...
### F-2.layout阶段-commitLayoutEffects主要做了两件事
...
### F-4.commitLayoutEffects第二个主要的是调用commitAttachRef
...
### F-4.至此，整个layout阶段就结束了,最后是current Fiber树切换
...
layout阶段的最后会判断rootDoesHavePassiveEffects，即看看是否有未处理的副作用，有则将rootWithPendingPassiveEffects赋值为root（整个应用的根节点），这有什么用呢？

让我们回到commitRootImpl的方法开头，它会循环判断rootWithPendingPassiveEffects，当其不为null时，执行flushPassiveEffects

### G-1.总结flushPassiveEffects 的功能
...
### F-2.react 17之后：合成事件:ReactDOMEventListeners.js
