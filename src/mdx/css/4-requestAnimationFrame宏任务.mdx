...
requestAnimationFrame 是一个宏任务。它用于在浏览器下一次重绘之前执行指定的回调函数，从而实现流畅的动画效果。根据 JavaScript 的任务分类，宏任务包括整体代码、setTimeout、setInterval 等，而微任务则包括 Promise 的回调和 MutationObserver。
...
## 使用requestAnimationFrame-setTimeout
...
<CodePannel name='requestAnimationFrame-setTimeout' language='html' code={testCode} height='500px'/>


## requestAnimationFrame-setTimeout的区别
...
<CodePannel name='requestAnimationFrame-setTimeout的区别' language='html' code={testCode2} height='500px'/>